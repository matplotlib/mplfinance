...
def plot(data, **kwargs):
    ...
    if config['marketcolor_overrides'] is not None:
        if len(config['marketcolor_overrides']) != len(dates):
            raise ValueError(
                '`marketcolor_overrides` must be same length as dataframe.')
    ...
    if external_axes_mode:
        ...
        if config['figscale'] is None:
            config['figscale'] = 1.0
        if config['figratio'] is None:
            config['figratio'] = DEFAULT_FIGRATIO
    ...
    if external_axes_mode:
        ...
        if config['volume']:
            if config['volume_panel'] == config['main_panel']:
                ...
                # Make sure ohlc is above volume
                volumeAxes.set_zorder(axA1.get_zorder()-0.1)
                # Let volume show through
                axA1.patch.set_visible(False)
    ...
    if ptype == 'line':
        ...
        collections = _construct_mpf_collections(
            ptype, dates, xdates, opens, highs, lows, closes, volumes, config, style)
    ...
    if config['return_calculated_values'] is not None:
        ...
        if config['mav'] is not None:
            ...
            if len(mav) != len(mavprices):
                warnings.warn('len(mav)='+str(len(mav)) +
                              ' BUT len(mavprices)='+str(len(mavprices)))
            else:
        ...
        if config['ema'] is not None:
            ...
            if len(ema) != len(emaprices):
                warnings.warn('len(ema)='+str(len(ema)) +
                              ' BUT len(emaprices)='+str(len(emaprices)))
            else:
    ...
    line_collections.append(_construct_vline_collections(
        config['vlines'], dtix, miny, maxy))
    ...
    for tline_item in tlines:
        line_collections.append(_construct_tline_collections(
            tline_item, dtix, dates, opens, highs, lows, closes))
    ...
    if config['volume']:
        ...
        volumeAxes.bar(xdates, volumes, width=w, linewidth=lw,
                       color=vcolors, ec=adjc, alpha=valp)
    ...
    if addplot is not None and ptype not in VALID_PMOVE_TYPES:
        ...
        if not external_axes_mode:
            ...
            panels.at[config['main_panel'], 'mag'] = {
                'lo': lo, 'hi': hi}  # update main panel magnitude range

            if config['volume']:
                lo = math.log(
                    max(math.fabs(np.nanmin(volumes)), 1e-7), 10) - 0.5
                hi = math.log(
                    max(math.fabs(np.nanmax(volumes)), 1e-7), 10) + 0.5
        ...
        for apdict in addplot:
            ...
            if not external_axes_mode:
                if panid == 'main':
                    panid = 0  # for backwards compatibility
                elif panid == 'lower':
                    panid = 1  # for backwards compatibility
            ...
            if aptype == 'ohlc' or aptype == 'candle':
                ax = _addplot_collections(
                    panid, panels, apdict, xdates, config)
                ...
                for column in apdata:
                    ...
                    ax = _addplot_columns(
                        panid, panels, ydata, apdict, xdates, config)
                    _addplot_apply_supplements(ax, apdict, xdates)
    ...
    if config['volume']:
        ...
        if vscale == 'linear':
            ...
            if vxp == 'legacy':
                ...
                volumeAxes.ticklabel_format(
                    useOffset=False, scilimits=(vxp, vxp), axis='y')
                offset = '  $10^{'+str(vxp)+'}$'
            elif isinstance(vxp, int) and vxp == 0:
                volumeAxes.ticklabel_format(
                ...
                if scilims[0] < scilims[1]:
                    for power in (5, 4, 3, 2, 1):
                        ...
                        if vymax >= 10.**xp:
                            volumeAxes.ticklabel_format(
                                useOffset=False, scilimits=(xp, xp), axis='y')
                            offset = '  $10^{'+str(xp)+'}$'
                            break
                elif scilims[0] == scilims[1] and scilims[1] != 0:
                    volumeAxes.ticklabel_format(
                        useOffset=False, scilimits=scilims, axis='y')
    ...
    if config['title'] is not None:
        ...
        if isinstance(config['title'], dict):
            ...
            # allows override default values set by mplfinance above
            title_kwargs.update(title_dict)
        else:
...
def _adjust_figsize(fig, config):
    ...
    if config['figsize'] is None:
        ...
        if r < 0.20 or r > 5.0:
            raise ValueError(
                '"figratio" (aspect ratio)  must be between 0.20 and 5.0 (but is '+str(r)+')')
...
def _addplot_collections(panid, panels, apdict, xdates, config):
    ...
    if aptype not in valid_apc_types:
        raise TypeError('Invalid aptype='+str(aptype) +
                        '. Must be one of '+str(valid_apc_types))
    if not isinstance(apdata, pd.DataFrame):
        raise TypeError('addplot type "'+aptype +
    ...
    collections = _construct_mpf_collections(
        aptype, d, xdates, o, h, l, c, v, config, apstyle)
    ...
    # if apdict['ema'] is not None:
    #     apemaprices = _plot_ema(ax, config, xdates, c, apdict['ema'])
...
def _addplot_columns(panid, panels, ydata, apdict, xdates, config):
    ...
    if aptype == 'scatter':
        ...
        color = apdict['color']
        alpha = apdict['alpha']
        ax.bar(xdates, ydata, width=width,
               bottom=bottom, color=color, alpha=alpha)
    elif aptype == 'line':
        ls = apdict['linestyle']
        color = apdict['color']
        width = apdict['width'] if apdict['width'] is not None else 1.6 * \
            config['_width_config']['line_width']
        alpha = apdict['alpha']
        ...
        width = apdict['width'] if apdict['width'] is not None else 1.6 * \
            config['_width_config']['line_width']
        alpha = apdict['alpha']
        ax.step(xdates, ydata, where=stepwhere, linestyle=ls,
    ...
    # if apdict['ema'] is not None:
    #     apemaprices = _plot_ema(ax, config, xdates, ydata, apdict['ema'])
    return ax
...
def _addplot_apply_supplements(ax, apdict, xdates):
    ...
    if "fill_between" in apdict and apdict['fill_between'] is not None:
        ...
        if _list_of_dict(fblist):
            ...
            raise ValueError(
                'Invalid addplot fill_between: must be `dict` or `list of dict`')
